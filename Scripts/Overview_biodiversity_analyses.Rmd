...
load("../../infauna_epifauna_merge_group_analysis_24022025.RData") # Data grouped at class/order/family level
load("../../infauna_epifauna_analysis_24022025.RData") # Data at genus/species level 
...
# Site with missing month (Tvärminne 1971) assumed to be 8
all_dat_g_env$Month[all_dat_g_env$Year==1971] <- 8
all_dat_env$Month[all_dat_env$Year==1971] <- 8
...
```{r fig_months_years, fig.cap="Distribution of observations from all sites over time. Size of the points indicate the number of months sampled that year, with color indicating which month.", fig.height=6, fig.width=10}
...
Simple overviews of total abundance, richness, diversity of overall groups of epifauna (class, order, family).
...
```{r heatmap_genus, eval=TRUE, fig.cap="Heat map of species abundance (log-transformed), only including sites with data collected after 2010.", fig.height=10, fig.width=10}

# Reshape data
dat_heat <- all_dat_env %>% 
  #Remove missing data, select relevant data
